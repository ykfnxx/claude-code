...
| 章节 | 文件 | 内容 |
|------|------|------|
| 第一章 | [[01-整体架构概览]] | 入口点、初始化流程、模块组织、特性标志 |
| 第二章 | [[02-核心引擎分析]] | QueryEngine、Query Loop、消息处理管道 |
| 第三章 | [[03-prompt-management]] | Prompt管理、系统提示词组装、Prompt工程技巧 |
| 第四章 | [[04-任务管理机制]] | 任务生命周期、多Agent协调、Agent间通信 |
| 第五章 | [[05-agent状态轮转流程]] | Agent状态机、查询循环转换、Terminal/Continue原因 |
| 第六章 | [[06-LLM调用兜底约束机制]] | 重试逻辑、模型降级、压缩机制、断路器 |
| 第七章 | [[07-工具系统]] | 工具注册、验证管道、执行引擎、权限系统 |
| 第八章 | [[08-状态管理]] | 全局状态、响应式Store、AppState字段详解 |
| 第九章 | [[09-命令系统与服务层]] | Slash命令系统、服务层架构、MCP/LSP/OAuth |
| 总结 | [[10-Harness工程总结]] | 设计哲学、关键技术决策、架构权衡 |
| 深度分析 | [[11-上下文管理机制深度分析]] | 上下文重组、压缩管道、缓存机制 |
...
│         │                │                │                     │
│         ↓                │                ↓                     │
│  [[08-状态管理]]          │        [[11-上下文管理机制深度分析]]  │
│         │                │                ↑                     │
│         │                │                │                     │
│  [[09-命令系统与服务层]]   │        [[03-prompt-management]]      │
│         │                │                │                     │
│         │                └───────────────┬┘                     │
│         │                                │                      │
│         └────────────────┬───────────────┘                      │
│                          ↓                                       │
│  [[04-任务管理机制]] ←→ [[05-agent状态轮转流程]]                  │
│                          │                                       │
│                          ↓                                       │
│  [[07-工具系统]] ←────→ [[06-LLM调用兜底约束机制]]                │
│                                                                   │
│  [[10-Harness工程总结]] (汇总所有章节)                            │
│                                                                   │
└─────────────────────────────────────────────────────────────────┘
```
...
- **入门阅读**: [[01-整体架构概览]] → [[02-核心引擎分析]]
- **Prompt 深度**: [[03-prompt-management]] → [[11-上下文管理机制深度分析]]
- **Agent 机制**: [[04-任务管理机制]] → [[05-agent状态轮转流程]]
- **系统保障**: [[06-LLM调用兜底约束机制]] → [[07-工具系统]]
- **工程总结**: [[10-Harness工程总结]]
